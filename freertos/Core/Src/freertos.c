...
void update_wheel_position_and_velocity(float *wheel_angle,
...
void StartControlLoop(void const *argument) {
	...
	for (;;) {
		...
		for (;;) {
			...
			float inertia_force = calculate_inertia(force_feedback_signal,
					angular_velocity);
			...
			total_force = force_feedback_signal + inertia_force + damping_force
					+ friction_force + lock_force;
...
float calculate_inertia(float force_feedback, float angular_velocity) {
	...
	float inertia_force = inertia_coefficient * previous_output
			+ (1 - inertia_coefficient) * force_feedback;
...
uint8_t map_wheel_position_to_axis(int32_t position) {
	...
	// Reverse the mapping
	return (uint8_t) ((((max_position - position) * 255)
			+ (max_position - min_position) / 2) / (max_position - min_position));
}
...
void update_wheel_position_and_velocity(float *wheel_angle,
	...
	*wheel_angle += delta_angle;
	if (*wheel_angle > WHEEL_MAX_ANGLE)
		*wheel_angle = WHEEL_MAX_ANGLE;
	if (*wheel_angle < -WHEEL_MAX_ANGLE)
		*wheel_angle = -WHEEL_MAX_ANGLE;
...
void motor_rotate_left() {
	uint32_t target_position = gPosition
			- (MAX_REVOLUTIONS * ENCODER_RESOLUTION);
...
void processCAN() {
	...
	while (HAL_CAN_GetRxFifoFillLevel(&hcan1, CAN_RX_FIFO1) > 0) {
		if (HAL_CAN_GetRxMessage(&hcan1, CAN_RX_FIFO1, &RxHeader, RxData)
			...
			// Steering Wheel
			case 0x200:
				user_input_data.buttons = (uint32_t) value;
				gDebugCounter1++;
				break;
			case 0x201:
				user_input_data.hall_analog_1 = (uint8_t) value;
				gDebugCounter1++;
				break;
			case 0x202:
				user_input_data.hall_analog_2 = (uint8_t) value;
				gDebugCounter1++;
				break;

				// Pedals
			case 0x300:
				pedal_data.encoder_1 = (int16_t) value;
				gDebugCounter2++;
				break;
			case 0x301:
				pedal_data.encoder_2 = (int16_t) value;
				gDebugCounter2++;
				break;
			case 0x302:
				pedal_data.encoder_3 = (int16_t) value;
				gDebugCounter2++;
				break;
			default:
				break;
