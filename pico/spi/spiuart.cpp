...
#define TX_PIN 0  // GPIO pin for TX
#define RX_PIN 1  // GPIO pin for RX
...
// Function to calculate the next oscillation value
int oscillate_value() {
...
int main()
{
    ...
    while (1) {
        // Get the next oscillation value
        int osc_value = oscillate_value();

        // Format the command for the rpmbar object
        char command[32];
        sprintf(command, "rpmbar.val=%d", osc_value);

        // Send the command to the Nextion HMI
